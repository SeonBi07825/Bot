...
def init():
	...
	basicSetting.append(inputData[12][14:])    #basicSetting[11] : !q 표시 보스 수
	basicSetting.append(inputData[14][16:])    #basicSetting[12] : restart 주기
	basicSetting.append(inputData[6][17:])     #basicSetting[13] : 정산 채널 ID
	basicSetting.append(inputData[15][12:])    #basicSetting[14] : 스프레드시트 파일 이름
	basicSetting.append(inputData[16][11:])    #basicSetting[15] : json 파일명
	basicSetting.append(inputData[17][12:])    #basicSetting[16] : 시트 이름
	basicSetting.append(inputData[18][12:])    #basicSetting[17] : 입력 셀
	basicSetting.append(inputData[19][13:])    #basicSetting[18] : 출력 셀
	basicSetting.append(inputData[11][13:])    #basicSetting[19] : 멍삭제횟수
					...
					if (bossTime[i]+datetime.timedelta(days=-365)) <= aftr:
						...
						if basicSetting[2] != '0':
							if int(basicSetting[19]) <= bossMungCnt[i] and int(basicSetting[19]) != 0:
								...
								bossMungFlag[i] = (False)
								bossMungCnt[i] = 0
								await client.get_channel(channel).send('```자동 멍처리 횟수 ' + basicSetting[19] + '회 초과! [' + bossData[i][0] + '] 삭제!```', tts=False)
								await dbSave()
...
while True:
	...
	async def on_message(msg):
		...
		if client.get_channel(channel) != msg.channel :
			...
			if basicSetting[10] != "":
				if msg.channel.id == int(basicSetting[10]):
					...
					if message.content.startswith(command[12]):
						ladder = []
						ladder = message.content[len(command[12])+1:].split(" ")
			...
			################ 정산 특정 채널에서 하기 ################ 
			if basicSetting[13] != "":
				if msg.channel.id == int(basicSetting[13]) : #### 정산채널 채널ID 값넣으면 됨
					message = await msg.channel.fetch_message(msg.id)
					...
					if message.content.startswith(command[13]):
						if basicSetting[14] != "" and basicSetting[15] != "" and basicSetting[16] != "" and basicSetting[17] != "" and basicSetting[18] != "":
							SearchID = message.content[len(command[13])+1:]
							gc = gspread.authorize(credentials)
							wks = gc.open(basicSetting[14]).worksheet(basicSetting[16])
			...
			################ 킬 확인 특정 채널에서 하기 ################ 
			if basicSetting[20] != "":
				if msg.channel.id == int(basicSetting[20]) : #### 킬 채널ID 값넣으면 됨
					message = await msg.channel.fetch_message(msg.id)

					################ 킬초기화 ################ 
					if message.content == command[27]:
						await initkill_list()
						await msg.channel.send( '< 킬 목록 초기화완료 >', tts=False)

					################ 킬명단 확인 ################ 
					if message.content == command[28]:
						kill_output = ''
					...
					################ 킬등록 ################ 
					if message.content.startswith(command[28]+' '):
						tmp_sayMessage = message.content
						sayMessage = tmp_sayMessage[len(command[28])+1:]
					...
					################ 킬삭제 ################ 
					if message.content.startswith(command[29]+' '):
						tmp_sayMessage = message.content
						sayMessage = tmp_sayMessage[len(command[29])+1:]
			...
			if message.content.startswith(command[4]):
				...
				for i in range(len(channel_name)):
					if  channel_name[i] == str(tmp_sayMessage1[len(command[4])+1:]):
			...
			if message.content.startswith(command[11]):
				separate_money = []
				separate_money = message.content[len(command[11])+1:].split(" ")
			...
			if message.content == command[0]:
				...
				command_list += command[4] + ' [채널명]\n'     #!채널이동
				command_list += command[5] + '\n'     #!소환
				command_list += command[6] + '\n'     #!불러오기
				command_list += command[7] + '\n'     #!초기화
				command_list += command[8] + '\n'     #!명치
				command_list += command[9] + '\n'     #!재시작
				command_list += command[10] + '\n'     #!미예약
				command_list += command[11] + ' [인원] [금액]\n'     #!분배
				command_list += command[12] + ' [뽑을인원수] [아이디1] [아이디2]...\n'     #!사다리
				command_list += command[13] + ' [아이디]\n'     #!정산
				command_list += command[14] + ' 또는 ' + command[14] + '0000, 00:00\n'     #!보스일괄
				command_list += command[15] + '\n'     #!카톡끔
				command_list += command[16] + '\n'     #!카톡켬
				command_list += command[17] + '\n'     #!q
				command_list += command[18] + ' [할말]\n'     #!k
				command_list += command[19] + ' [할말]\n'     #!v
				command_list += command[20] + '\n'     #!카톡보스
				command_list += command[21] + '\n'     #!리젠
				command_list += command[22] + '\n'     #!현재시간
				command_list += command[27] + '\n'     #!킬초기화
				command_list += command[28] + '\n'     #!킬횟수 확인
				command_list += command[28] + ' [아이디]\n'     #!킬
				command_list += command[29] + ' [아이디]\n'     #!킬삭제
				command_list += command[23] + '\n'     #!공지
				command_list += command[23] + ' [공지내용]\n'     #!공지
				command_list += command[23] + '삭제\n'     #!공지
				command_list += command[24] + ' [할말]\n\n'     #!상태
				command_list += command[25] + '\n'     #보스탐
				command_list += command[26] + '\n'     #!보스탐
			...
			if message.content.startswith(command[18]) or message.content.startswith('!ㅏ') or message.content.startswith('!K'): 
				tmp_sayMessage = message.content
			...
			if message.content == command[9]:
				...
				#await FixedBossDateSave()
				await voice_client1.disconnect()
				#await client.get_channel(channel).send( '< 보탐봇 재시작 중! 잠시만요! >', tts=False)
				print("보탐봇강제재시작!")
				os.system('restart.bat')
				#sys.exit()
			...
			if message.content.startswith(command[14]):
				for i in range(bossNum):
					tmp_msg = command[14]
					if len(hello) > len(tmp_msg) + 3 :
						if hello.find(':') != -1 :
			...
			if message.content == command[1]:			
				setting_val = '보탐봇버전 : PC Ver.22 (2020. 2. 10.)\n'
			...
			if message.content == '!ㅂ' or message.content == command[17] or message.content == '!ㅃ' or message.content == '!Q':
			...
			if message.content == command[25] or message.content == '/1' or message.content == '/보스' :
			...
			if message.content.startswith(command[13]):
				if basicSetting[14] != "" and basicSetting[15] != "" and basicSetting[16] != "" and basicSetting[17] != "" and basicSetting[18] != "":
					SearchID = hello[len(command[13])+1:]
					gc = gspread.authorize(credentials)
